...
describe("POST /api/services", () => {
  test("creates a new service", async () => {
    const res = await asAdmin(request(app).post("/api/services")).send({
  ...
  test("rejects missing name", async () => {
    const res = await asAdmin(request(app).post("/api/services")).send({ price: 30, duration: 20 });
    expect(res.status).toBe(400);
    expect(res.body.errors).toBeDefined();
  });

  test("rejects negative price", async () => {
    const res = await asAdmin(request(app).post("/api/services")).send({ name: "Waxing", price: -10, duration: 20 });
    expect(res.status).toBe(400);
  });

  test("rejects zero price", async () => {
    const res = await asAdmin(request(app).post("/api/services")).send({ name: "Waxing", price: 0, duration: 20 });
    expect(res.status).toBe(400);
  });

  test("rejects decimal price", async () => {
    const res = await asAdmin(request(app).post("/api/services")).send({ name: "Waxing", price: 9.99, duration: 20 });
    expect(res.status).toBe(400);
  });

  test("rejects duration over 480", async () => {
    const res = await asAdmin(request(app).post("/api/services")).send({ name: "Waxing", price: 30, duration: 500 });
    expect(res.status).toBe(400);
  });

  test("rejects duplicate name", async () => {
    const res = await asAdmin(request(app).post("/api/services")).send({ name: "Facial", price: 30, duration: 20 });
    expect(res.status).toBe(409);
  });

  test("rejects name with numbers", async () => {
    const res = await asAdmin(request(app).post("/api/services")).send({ name: "Facial123", price: 30, duration: 20 });
    expect(res.status).toBe(400);
  });
...
describe("PUT /api/services/:id", () => {
  test("updates a service", async () => {
    const res = await asAdmin(request(app).put("/api/services/1")).send({ price: 60 });
    expect(res.status).toBe(200);
    expect(res.body.price).toBe(60);
  });

  test("returns 404 for non-existent id", async () => {
    const res = await asAdmin(request(app).put("/api/services/999")).send({ price: 60 });
    expect(res.status).toBe(404);
  });

  test("rejects invalid price on update", async () => {
    const res = await asAdmin(request(app).put("/api/services/1")).send({ price: -5 });
    expect(res.status).toBe(400);
  });

  test("rejects duplicate name on update", async () => {
    const res = await asAdmin(request(app).put("/api/services/1")).send({ name: "Haircut" });
    expect(res.status).toBe(409);
  });

  test("allows same name on same service", async () => {
    const res = await asAdmin(request(app).put("/api/services/1")).send({ name: "Facial", price: 60 });
    expect(res.status).toBe(200);
  });
});
...
describe("DELETE /api/services/:id", () => {
  test("deletes a service", async () => {
    const res = await asAdmin(request(app).delete("/api/services/1"));
  ...
  test("returns 404 for non-existent id", async () => {
    const res = await asAdmin(request(app).delete("/api/services/999"));
    expect(res.status).toBe(404);
  });

  test("returns 400 for invalid id", async () => {
    const res = await asAdmin(request(app).delete("/api/services/abc"));
    expect(res.status).toBe(400);
  });
});
...
describe("GET /api/services/statistics", () => {
  test("returns statistics", async () => {
    const res = await asAdmin(request(app).get("/api/services/statistics"));
    expect(res.status).toBe(200);
...
describe("Appointment persistence and relational queries", () => {
  test("creates an appointment linked to an existing service", async () => {
    const res = await asClient(request(app).post("/api/appointments")).send({
      service: "Facial",
      serviceId: 1,
      date: "2026-05-10",
      time: "10:00 AM",
  ...
  test("filters appointments by email", async () => {
    await asClient(request(app).post("/api/appointments")).send({
      service: "Facial",
      serviceId: 1,
      date: "2026-05-10",
      time: "10:00 AM",
    ...
    const res = await asClient(request(app).get("/api/appointments?email=client@example.com"));
  ...
  test("counts and updates appointments for a service", async () => {
    const created = await asClient(request(app).post("/api/appointments")).send({
      service: "Facial",
      serviceId: 1,
      date: "2026-05-10",
      time: "10:00 AM",
    ...
    const count = await asClient(request(app).get("/api/services/1/appointment-count"));
    expect(count.status).toBe(200);
    expect(count.body.count).toBe(1);

    const updated = await asAdmin(
      request(app).put(`/api/services/1/appointments/${created.body.id}/status`)
  ...
  test("deletes an appointment", async () => {
    ...
    const deleted = await asClient(request(app).delete(`/api/appointments/${created.body.id}`));
...
describe("Client persistence", () => {
  test("returns paginated clients", async () => {
    const res = await asAdmin(request(app).get("/api/clients"));
    expect(res.status).toBe(200);
    expect(res.body.data).toHaveLength(1);
  ...
  test("creates a client", async () => {
    const res = await asAdmin(request(app).post("/api/clients")).send({
      name: "Ana Pop",
  ...
  test("filters clients by search", async () => {
    await asAdmin(request(app).post("/api/clients")).send({
      name: "Ana Pop",
      email: "ana@example.com",
    });

    const res = await asAdmin(request(app).get("/api/clients?search=ana"));
    expect(res.status).toBe(200);
    expect(res.body.data).toHaveLength(1);
  ...
  test("updates a client", async () => {
    const created = await asAdmin(request(app).post("/api/clients")).send({
      name: "Ana Pop",
      email: "ana@example.com",
    });

    const res = await asAdmin(request(app).put(`/api/clients/${created.body.id}`)).send({ phone: "0722222222" });
  ...
  test("deletes a client", async () => {
    ...
    const deleted = await asAdmin(request(app).delete(`/api/clients/${created.body.id}`));
    expect(deleted.status).toBe(204);

    const check = await asAdmin(request(app).get(`/api/clients/${created.body.id}`));
    expect(check.status).toBe(404);
  });
...
describe("NoSQL activity endpoint", () => {
  test("reports MongoDB activity status without breaking the relational API", async () => {
    const res = await asClient(request(app).get("/api/activity"));
