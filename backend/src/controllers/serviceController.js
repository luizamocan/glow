...
const getServiceById = async (req, res) => {
  ...
  const service = await store.getById(id);
...
const createService = async (req, res) => {
  ...
  const exists = await store.getByName(req.body.name.trim());
...
const updateService = async (req, res) => {
  ...
  if (req.body.name) {
    const duplicate = await store.getByName(req.body.name.trim());
    if (duplicate && duplicate.id === id) {
      return res.json(await store.update(id, {
...
const getServiceAppointments = async (req, res) => {
  const {id} = req.params;
  const appointments = await appointmentStore.getByServiceId(id);
...
const getServiceAppointmentCount = async (req, res) => {
  const {id} = req.params;
  const count = await appointmentStore.countByServiceId(id);
  res.json({count});
};
...
const updateAppointmentStatus = async (req, res) => {
  ...
  const updated= await appointmentStore.updateStatus(appId, status);
  if (!updated) {
